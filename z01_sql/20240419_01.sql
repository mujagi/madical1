...
no number(4,2),
mdate date,
mdate2 timestamp -- date = 년,월,일,시,분,초 까지 ㅈ장 timestamp 밀리초까지 저장가능
);
...
-- 사원번호(e id), 사원이림(e name),급여(salary),입사일자(hire_date)
...
-- 커미션은 실제월급 =월급 + 월급*커미션
select *from employees;
select employee_id,emp_name,salary,salary*commission_pct,salary+(salary*commission_pct) from employees;
